...
describe('parse-prd OpenAI integration', () => {
	test('preferOpenAI parameter is properly defined', () => {
		// This simulates the parameter definition in the tool registration
		const parameter = {
			name: 'preferOpenAI',
			description: 'Prefer using OpenAI over Claude for task generation (requires OPENAI_API_KEY).',
			type: 'boolean',
			default: false
		};

		expect(parameter.name).toBe('preferOpenAI');
		expect(parameter.type).toBe('boolean');
		expect(parameter.default).toBe(false);
		expect(parameter.description).toContain('OpenAI over Claude');
	});

	test('Session environment is updated when preferOpenAI is set', () => {
		// Mock SessionEnv update function
		...
		// Simulate the function that processes the preferOpenAI parameter
		const processParameters = (params, session) => {
			if (params.preferOpenAI) {
				session.env.set('PREFER_OPENAI', 'true');
			}
			return { success: true };
		};

		// Test with preferOpenAI set to true
		const resultWithPrefer = processParameters({ preferOpenAI: true }, mockSession);
		expect(mockSetEnv).toHaveBeenCalledWith('PREFER_OPENAI', 'true');
		expect(resultWithPrefer.success).toBe(true);

		// Reset mock
		mockSetEnv.mockClear();

		// Test with preferOpenAI not set
		const resultWithoutPrefer = processParameters({}, mockSession);
		expect(mockSetEnv).not.toHaveBeenCalled();
		expect(resultWithoutPrefer.success).toBe(true);
	});
});
