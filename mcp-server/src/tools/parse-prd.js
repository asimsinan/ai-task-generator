	...
	server.addTool({
		...
		parameters: z.object({
			...
			preferClaude: z
				...
				.optional()
				.default(false)
				.describe('Prefer using Claude over OpenAI for task generation (requires ANTHROPIC_API_KEY).'),
			projectRoot: z
				.string()
		...
		execute: async (args, { log, session }) => {
			try {
				...
				// Update session environment with preferClaude preference if specified
				if (args.preferClaude) {
					session.env = session.env || {};
					session.env.PREFER_CLAUDE = args.preferClaude ? 'true' : 'false';
					log.info(`Set Claude preference in session: ${args.preferClaude}`);
				}
