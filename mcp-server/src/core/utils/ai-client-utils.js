...
/**
 * Get an OpenAI client instance initialized with MCP session environment variables
 * @param {Object} [session] - Session object from MCP containing environment variables
 * @param {Object} [log] - Logger object to use (defaults to console)
 * @returns {OpenAI} OpenAI client instance
 * @throws {Error} If API key is missing or OpenAI package can't be imported
 */
export async function getOpenAIClientForMCP(session, log = console) {
	try {
		// Extract API key from session.env or fall back to environment variables
		const apiKey =
		...
		if (!apiKey) {
			throw new Error(
				'OPENAI_API_KEY not found in session environment or process.env'
			);
		}
		...
		// Initialize and return a new OpenAI client
		return new OpenAI({
			apiKey
		});
	} catch (error) {
...
/**
 * Get OpenAI model configuration from session environment or fall back to defaults
 * @param {Object} [session] - Session object from MCP containing environment variables 
 * @param {Object} [defaults] - Default model configuration to use if not in session
 * @returns {Object} Model configuration with model, maxTokens, and temperature
 */
export function getOpenAIModelConfig(session, defaults = DEFAULT_OPENAI_CONFIG) {
	// Get values from session or fall back to defaults
	return {
		model: session?.env?.OPENAI_MODEL || defaults.model,
		maxTokens: parseInt(session?.env?.OPENAI_MAX_TOKENS || defaults.maxTokens),
		temperature: parseFloat(session?.env?.OPENAI_TEMPERATURE || defaults.temperature)
	};
}
 ...
 * @param {boolean} [options.preferOpenAI=false] - Whether to prefer OpenAI over Claude when both are available
 * @param {Object} [log] - Logger object to use (defaults to console)
...
export async function getBestAvailableAIModel(
	...
	// Check for OpenAI when preferred
	if (
		preferOpenAI &&
		(session?.env?.OPENAI_API_KEY || process.env.OPENAI_API_KEY)
	) {
		try {
			...
			const client = await getOpenAIClientForMCP(session, log);
	...
	// Try OpenAI as backup when Claude is overloaded
	if (
		claudeOverloaded &&
		(session?.env?.OPENAI_API_KEY || process.env.OPENAI_API_KEY)
	) {
		try {
			log.info('Claude is overloaded, trying OpenAI as backup');
			const client = await getOpenAIClientForMCP(session, log);
			return { type: 'openai', client };
		} catch (error) {
			log.warn(`OpenAI not available as Claude backup: ${error.message}`);
			// Fall through to other options
		}
	}
	...
	if (session?.env?.OPENAI_API_KEY || process.env.OPENAI_API_KEY) {
		try {
			log.warn('Claude not available, falling back to OpenAI');
			const client = await getOpenAIClientForMCP(session, log);
			return { type: 'openai', client };
		} catch (error) {
			log.error(`OpenAI fallback not available: ${error.message}`);
...
export function handleOpenAIError(error) {
	// Check if it's a structured error response from OpenAI
	if (error.error) {
		switch (error.error.type) {
