...
function registerCommands(programInstance) {
	...
	programInstance
		...
		.option('--prefer-claude', 'Use Claude instead of OpenAI for task generation (requires ANTHROPIC_API_KEY)')
		.action(async (file, options) => {
			...
			const preferClaude = options.preferClaude || false;
			...
			if (!inputFile) {
				if (fs.existsSync(defaultPrdPath)) {
					...
					if (preferClaude) {
						console.log(chalk.blue('Using Claude for task generation'));
					}
					await parsePRD(defaultPrdPath, outputPath, numTasks, { preferClaude });
					return;
				}
			...
			await parsePRD(inputFile, outputPath, numTasks, { preferClaude });
		});
