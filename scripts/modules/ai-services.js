...
function getOpenAIClient() {
	if (!openai) {
		if (!process.env.OPENAI_API_KEY) {
			throw new Error(
				'OPENAI_API_KEY environment variable is missing. Set it to use OpenAI features.'
			);
		}
		openai = new OpenAI({
 ...
 * @param {boolean} options.preferOpenAI - Whether to prefer OpenAI over Claude
	...
	if (error.type === 'error' && error.error) {
		switch (error.error.type) {
			case 'overloaded_error':
				// Check if we can use Perplexity or OpenAI as a fallback
				if (process.env.OPENAI_API_KEY) {
					return 'Claude is currently overloaded. Trying to fall back to OpenAI.';
...
function handleOpenAIError(error) {
	...
	if (error.error) {
		switch (error.error.type) {
			...
			case 'insufficient_quota':
				return 'Your OpenAI quota has been exceeded. Please check your billing settings.';
			case 'invalid_request_error':
				return 'There was an issue with the OpenAI request format. If this persists, please report it as a bug.';
			default:
				return `OpenAI API error: ${error.error.message || error.message}`;
		}
	}
	...
	if (error.message?.toLowerCase().includes('network')) {
		return 'There was a network error connecting to OpenAI. Please check your internet connection and try again.';
	}
...
/**
 * Call OpenAI to generate tasks from a PRD
 * @param {string} prdContent - PRD content
 * @param {string} prdPath - Path to the PRD file
 * @param {number} numTasks - Number of tasks to generate
 * @param {number} retryCount - Retry count 
...
Expected output format:
{
  "tasks": [
    {
      "id": 1,
      "title": "Setup Project Repository",
      "description": "Create GitHub repository with initial project structure",
      "status": "pending",
      "dependencies": [],
      "priority": "high",
      "details": "Initialize with README, .gitignore, set up initial directory structure...",
  ...
  "metadata": {
    ...
    "totalTasks": ${numTasks},
    "sourceFile": "${prdPath}",
    "generatedAt": "YYYY-MM-DD" (current date)
  }
}`;
		...
		if (outputFormat === 'text' && !isSilentMode()) {
			loadingIndicator = startLoadingIndicator('Generating tasks from PRD with OpenAI...');
		}

		// Get the Anthropic client, either from params or session
		const client = aiClient || getOpenAIClient();
		const modelName = modelConfig?.model || process.env.OPENAI_MODEL || 'gpt-4o';
		const maxTokens = modelConfig?.maxTokens || parseInt(process.env.OPENAI_MAX_TOKENS || '4096');
		const temperature = modelConfig?.temperature || parseFloat(process.env.OPENAI_TEMPERATURE || '0.2');

		report(`Using OpenAI model: ${modelName}`, 'info');
		...
		const response = await client.chat.completions.create({
			...
			messages: [
				...
				{ 
					role: 'user', 
					content: `Here is a PRD to process into tasks: \n\n${prdContent}` 
				}
			],
		...
		const parsedResponse = processJsonResponse(
			textContent,
			numTasks,
			retryCount,
			prdContent,
		...
		// Handle API errors
		const userFriendlyError = handleOpenAIError(error);
		log('error', userFriendlyError);

		if (retryCount < 2) {
			log('info', `Retrying with OpenAI (${retryCount + 1}/2)...`);
			return callOpenAI(prdContent, prdPath, numTasks, retryCount + 1, {
				reportProgress,
				mcpLog,
				session
			});
		...
		throw new Error(
			`Failed to generate tasks with OpenAI after multiple attempts: ${userFriendlyError}`
		);
	}
}

/**
 * Process the JSON response from AI models
 * @param {string} textContent - Text content from the AI model
 * @param {number} numTasks - Number of tasks expected
 * @param {number} retryCount - Retry count
 * @param {string} prdContent - PRD content
 * @param {string} prdPath - Path to the PRD file
